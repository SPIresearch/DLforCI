...
def segment(in_files,BCI_glomeruli,BCI_tubule,GLI_gs,GLI_fc,IFTA_identifier,mask_generator):
    ...
    for i, fn in enumerate(os.listdir(in_files)): 
        ...
        glomeruli_mask=cv2.resize(glomeruli_mask,(IF_TA_mask.shape[1],IF_TA_mask.shape[0]))

        tubule_mask=cv2.resize(tubule_mask,(IF_TA_mask.shape[1],IF_TA_mask.shape[0]))
...
def get_args():
    ...
    parser.add_argument('--input', '-i', metavar='INPUT', nargs='+',
                        help='filenames of input images', default='./imgs')
...
if __name__ == "__main__":
    ...
    segment(in_files,BCI_glomeruli,BCI_tubule,GLI_gs,GLI_fc,IFTA_identifier,mask_generator)
