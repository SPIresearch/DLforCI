...
def segment_IFTA(img, IFTA_identifier, glomeruli_mask, tubule_mask):
    mask_ifta = identify_IFTA_region(
        net=IFTA_identifier, full_img=img, nclasses=2, scale_factor=4, weight=[0.1, 0.9]
    )
    ...
    IF_TA_mask = distinguish_IF_TA(
        ...
        mask_ifta=mask_ifta,
        patch_size=2048,
        glomeruli_mask=glomeruli_mask,
        nclasses=3,
        scale_factor=2,
    )

    IF_TA_mask = cv2.resize(
        IF_TA_mask,
        (mask_ifta.shape[1], mask_ifta.shape[0]),
        interpolation=cv2.INTER_AREA,
    )
    return IF_TA_mask


def identify_IFTA_region(
    net, full_img, scale_factor=4, nclasses=2, patch_size=1024, weight=[1, 1, 1.3]
):
    full_img = cv2.resize(
        full_img, (full_img.shape[1] // scale_factor, full_img.shape[0] // scale_factor)
    )

    h_p, w_p = (
        patch_size - full_img.shape[0] % patch_size,
        patch_size - full_img.shape[1] % patch_size,
    )
    full_img = cv2.copyMakeBorder(
        full_img, 0, h_p, w_p, 0, cv2.BORDER_CONSTANT, value=0
    )
    ...
    for y, x in point_set:
        ...
        with torch.no_grad():
            ...
            mask_image[:, y : y + patch_size, x : x + patch_size] = (
...
def distinguish_IF_TA(
    ...
    glomeruli_mask = cv2.copyMakeBorder(
        glomeruli_mask, 0, h_p, w_p, 0, cv2.BORDER_CONSTANT, value=0
    )
    tubule_mask = cv2.copyMakeBorder(
        tubule_mask, 0, h_p, w_p, 0, cv2.BORDER_CONSTANT, value=0
    )
    mask_ifta = cv2.copyMakeBorder(
        mask_ifta, 0, h_p, w_p, 0, cv2.BORDER_CONSTANT, value=0
    )
...
def cal_if_ci(glomeruli_mask, img, IF_TA_mask):
    ...
    mask_the_whole = cv2.resize(
        mask_the_whole,
        (IF_TA_mask.shape[1], IF_TA_mask.shape[0]),
        interpolation=cv2.INTER_AREA,
    )
...
label2color_d = {
    "background": [0, 0, 0],
    "GS": [255, 255, 0],
    "other_glomeruli": [0, 0, 255],
    "FC": [0, 255, 0],
    "normal_tubules": [255, 0, 128],
    "TA": [255, 0, 0],
    "IF": [255, 235, 205],
}
...
def add_mask_img(img, mask):
    ...
    img1 = img * (1 - mask1)
    img = (img * 0.3 + mask * 0.7) + img1 * 0.7
    img = cv2.resize(img * 1.0, (img.shape[1] // 2 + 1, img.shape[0] // 2 + 1)).astype(
        np.int
    )
    return img
...
def filter_connectedComponents(mask_glomeruli):
    ...
    ccNum, labels, stats, centroids = cv2.connectedComponentsWithStats(
        mask_glomeruli.astype(np.uint8)
    )
...
def the_whole_area(gray, counter_thre=30):
    ...
    while True:
        ret, binary = cv2.threshold(
            detected_edges, counter_thre, 255, cv2.THRESH_BINARY
        )
        counter_thre += 30
    ...
    for y, x in point_set:
        ...
        if sam_refine:
            ...
            for s_mask in sam_mask:
                ...
                elif area >= img.shape[2] * img.shape[3] / 100:
...
def segment_glomeruli(img, BCI_glomeruli, mask_generator):
    mask_glomeruli = predict_img_glomeruli(
        net=BCI_glomeruli,
        full_img=img,
...
def identify_gs_fc(mask, img, GLI_gs, GLI_fc):
    mask = cv2.resize(
        mask, (mask.shape[1] * 4, mask.shape[0] * 4), interpolation=cv2.INTER_NEAREST
    )
